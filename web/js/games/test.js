  ...
  return e(
    'div',
    {
      className: `game-player ${
        isAnswering ? (isDone ? 'game-done' : 'game-not-done') : ''
      } ${className}`,
    },
      ...
      e(
        'span',
        {
          className: `game-player-score ${
            user.scoreChange > 0 ? 'game-player-score-changed' : ''
          }`,
        },
        score
      ),
    user.scoreChange > 0 &&
      e(
        'span',
        { className: 'game-player-score-change' },
        '+' + user.scoreChange
      )
  );
}
...
function Game({ channel, onEnd, questions }) {
  ...
  return e(
    ...
    gameState.type === 'answering'
      ...
      : gameState.type === 'voting'
      ? e(
          // VOTING CARD COMPARISON
          React.Fragment,
          null,
          e('p', { className: 'game-prompt big-bold' }, gameState.prompt),
          e(
            'div',
            {
              className: `game-card game-vote-candidate game-left ${
                gameState.aVotes &&
                gameState.aVotes.length > gameState.bVotes.length
                  ? 'game-winner'
                  : ''
              } ${
                gameState.aVotes &&
            ...
            gameState.userA &&
              e(User, {
                className: 'game-answer-author',
                user: gameState.userA,
              }),
            ...
            e(
              ...
              gameState.aVotes &&
                gameState.aVotes.map((user) => e(User, { user, key: user.id }))
            )
          ),
          e(
            'div',
            ...
            e(
              ...
              { className: 'game-answer-voters' },
              gameState.bVotes &&
                gameState.bVotes.map((user) => e(User, { user, key: user.id }))
            )
          ),
          gameState.aVotes &&
            gameState.aVotes.length === gameState.bVotes.length &&
            e('p', { className: 'game-tie big-bold' }, 'Tie!')
        )
      : e(
          // END SCOREBOARD
          'div',
      ...
      e(
        ...
        scores.map(([user, score]) =>
          e(User, {
            ...
            isDone:
              gameState.type === 'answering' &&
              gameState.done.includes(user.id),
            key: user.id,
          })
        )
      )
